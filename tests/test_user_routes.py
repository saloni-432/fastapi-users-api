...
class TestCreateUser:
    def test_creates_successfully(self, client):
        r = client.post("/users/", json={"name": "Bob", "email": "bob@test.com"})
        assert r.status_code == 200
...
class TestUpdateUser:
    def test_updates_user(self, client, existing_user):
        r = client.put(f"/users/{existing_user.id}", json={"name": "Updated", "email": "updated@test.com"})
        assert r.status_code == 200
        assert r.json()["name"] == "Updated"

    def test_missing_user(self, client):
        r = client.put("/users/999999", json={"name": "X", "email": "x@test.com"})
...
class TestDeleteUser:
    ...
    def test_user_gone_after_delete(self, client, existing_user):
        ...
        assert client.get(f"/users/{existing_user.id}").json() is None
